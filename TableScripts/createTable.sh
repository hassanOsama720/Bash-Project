...
while ! [[ $tableName =~ ^([a-zA-Z])[a-zA-Z0-9\w_-]*([a-zA-Z0-9])$ ]]; do
    ...
    echo "table names should not have any special characters, spaces";
    echo ""
    read -rp "Enter table name: " tableName;
done
...
function createTableFiles(){
    ...
    if touch "$currDB/$tableName.data" 2>> Error.out; then
        echo "Data table created sucessfully." >> Error.out;
        # create .tableName.frm
        if touch "$currDB/.$tableName.meta" 2>> Error.out; then
            echo "Format file created sucessfully." >> Error.out;
        else
            echo "Falied to create the format file. Check Error.out for more details.";
            success=1
        fi
    else
        echo "Falied to create data table. Check Error.out for more details.";
        success=1
    fi
...
function createColumns(){
    ...
    do
        ...
        echo $colMetadata >> "$currDB/.$tableName.meta";
        echo "Column format added $colMetadata" >> Error.out;
    done
}
...
# main
if test -f $currDB/$tableName 2> Error.out; then
	echo "Table already exists. Check Error.out for more details.";
else
